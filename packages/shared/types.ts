...
export const variantSchema = z.object({
  ...
  performance: z.record(z.string()), product_showcase:z.array(z.string()).min(1),
...
export type Task = { id:string; project_id:string; createdAt:string; updatedAt:string; requirement:string; assets:Asset[]; status:Status; provider:'mock'|'seedance'; director:'mock'|'pi'; logs:{time:string;message:string}[]; results:Result[]; metadata?:Metadata; plan?:Plan; error?:string };
