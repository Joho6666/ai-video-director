...
export const supplementsSchema=z.array(z.object({id:z.enum(['V1','V2','V3']),structure:structureSchema,product_showcase:z.array(z.string()).min(1)})).length(3);
export function checkPlan(plan:Plan){
 if(new Set(plan.variants.map(v=>v.id)).size!==3)throw new Error('Director 必须输出 V1 / V2 / V3');
 for(const v of plan.variants){for(let i=1;i<v.timeline.length;i++)if(v.timeline[i-1].end_state!==v.timeline[i].start_state)throw new Error(`${v.id} 动作时间线不连续`);if(Math.abs(v.timeline.reduce((a,s)=>a+s.duration,0)-8)>0.1)throw new Error('每个版本必须为 8 秒');}
 for(let i=0;i<3;i++)for(let j=i+1;j<3;j++){const changes=structureKeys.filter(k=>plan.variants[i].structure[k]!==plan.variants[j].structure[k]);if(changes.length<3)throw new Error('版本间不足三个结构差异');}
}
