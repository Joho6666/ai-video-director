...
function outputContract(){return `Return exactly one JSON object with keys treatment, supplements, reference_evidence. treatment MUST match the supplied Director output schema. Produce exactly V1,V2,V3 in INSPIRE mode. Each 8-second timeline has 2-4 continuous beats. The previous end_state must exactly equal the next start_state. Each variation must pass similarity guard. Seedance complete_prompt must be compact English under 1800 characters. supplements is an array of three {id,structure,product_showcase}; structure has exactly these string keys: ${Object.keys(structureSchema.shape).join(', ')}. Every variant must have 2-4 product_showcase items shaped {feature,action,camera_focus,evidence}; evidence names observable product image/user text or says Unknown. reference_evidence has exactly these keys: ${evidenceFields.join(', ')}. Every evidence value is {status:"Observed"|"Inferred"|"Unknown",description,frame_ids}. Observed requires evidence frames. Unknown uses []. Inferred description must explicitly say it is inferred/推断. Frame IDs must come from those provided. Sampled still frames do not prove the full path between frames: uncertain running gait, foot contact, fast hand movement, micro-expression or transitions MUST be Inferred or Unknown, never Observed. Never invent product materials, functions, measurements, camera numbers or unseen transitions. Prioritize current state, previous state, next state, transition, weight/support foot, asymmetric arms, sustained product contact, gaze → head → shoulders → torso delay, and clothing/hair/product settling.`;}
...
export class DeepSeekDirectorAdapter implements AgentAdapter {
 async plan(task:Task):Promise<{plan:Plan;treatment:unknown;evidence:unknown;requestMeta:unknown}>{
  ...
  const envelope=deepSeekEnvelopeSchema.parse(raw);validateEvidenceFrameIds(envelope.reference_evidence,new Set(input.frames.map(f=>f.id)));
