...
test('QC makes an inferred status explicit without changing its evidence',()=>{
 const value=result();Object.assign(value.evidence[0],{status:'inferred',description:'Motion continuity cannot be fully confirmed',confidence:'medium'});
 const report=validate(value);assert.match(report.evidence[0].description,/^Inferred:/);assert.deepEqual(report.evidence[0].frame_ids,['qc_frame_01']);
});
