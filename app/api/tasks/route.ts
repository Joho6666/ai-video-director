...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const fingerprint=createHash('sha256').update(requirement).update(config.appMode).update(JSON.stringify({selectedVariants,taskType,provider:providerPreference,model:config.videoProvider})).update(buffered.map(x=>createHash('sha256').update(x.data).digest('hex')).join(':')).digest('hex');
