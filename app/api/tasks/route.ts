...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const referenceImportId = String(form.get('referenceImportId') || '');
  if (!(reference instanceof File) && !/^[a-f0-9-]{36}$/.test(referenceImportId)) throw new Error('请上传参考视频或先导入参考链接');
  if (reference instanceof File && (!reference.size || !/\.(mp4|mov)$/i.test(reference.name) || reference.size>100*1024*1024)) throw new Error('参考视频需为 MP4 / MOV 且不超过 100 MB');
  ...
  const rawPreference=String(form.get('providerPreference')||'auto');
  if(!['auto','wan','minimax'].includes(rawPreference))throw new Error('Provider 选择无效');
  const providerPreference=rawPreference as ProviderPreference;
  ...
  const remixMode=String(form.get('remixMode')||'structure');if(!['creative','structure','close'].includes(remixMode))throw new Error('Remix 模式无效');
  ...
  const config=resolveAppConfig({...process.env, VIDEO_PROVIDER: providerPreference==='auto' ? process.env.VIDEO_PROVIDER : providerPreference});
  ...
  const incoming=[...(reference instanceof File ? [{file:reference,kind:'reference' as const}] : []),...models.map(file=>({file:file as File,kind:'model' as const})),...products.map(file=>({file:file as File,kind:'product' as const})),...(firstFrame instanceof File&&firstFrame.size?[{file:firstFrame,kind:'first_frame' as const}]:[])];
  const buffered=await Promise.all(incoming.map(async item=>({...item,data:Buffer.from(await item.file.arrayBuffer())})));
  const fingerprint=createHash('sha256').update(requirement).update(config.appMode).update(JSON.stringify({selectedVariants,taskType,provider:providerPreference,model:config.videoProvider,referenceImportId,remixMode})).update(buffered.map(x=>createHash('sha256').update(x.data).digest('hex')).join(':')).digest('hex');
  ...
  const task:Task={taskType:taskType as "fashion"|"ecommerce",selectedVariants,remixMode:remixMode as Task['remixMode'],id,project_id:id,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),requirement,assets:[],status:'UPLOADED',appMode:config.appMode,provider:config.videoProvider,providerPreference,director:config.director,idempotencyKey,logs:[{time:new Date().toISOString(),message:`素材已保存 · Provider 路线 ${providerPreference}` }],results:config.appMode==='director'?[]:selectedVariants.map((resultId,i)=>({id:resultId,name:['轻奢时尚','都市通勤','活力街拍'][i],status:'waiting'}))};
  ...
  if (referenceImportId) {
    const sourceDir = path.join(dataRoot, 'reference-imports', referenceImportId); const imported = JSON.parse(await readFile(path.join(sourceDir, 'reference.json'), 'utf8')); const filename = String(imported.localFile || ''); if (!/^reference-[a-f0-9-]+\.mp4$/.test(filename)) throw new Error('导入参考视频记录无效');
    const bytes = await readFile(path.join(sourceDir, filename)); const relative = `uploads/reference.mp4`; await writeFile(path.join(projectDir(id), relative), bytes); task.assets = task.assets.filter(asset => asset.kind !== 'reference'); task.assets.push({ name: 'reference.mp4', file: relative, mime: 'video/mp4', kind: 'reference', sourceUrl: imported.sourceUrl, sourceProvider: imported.provider, metadata: imported });
  }
