...
Web 只负责素材输入、状态轮询和结果展示。API 校验类型、大小、扩展名和配置，使用原子 JSON 写入保存任务。`video-analysis` 使用 ffprobe 获取 duration/fps/resolution，再以 FFmpeg 抽取 16 帧、时间顺序清单和联系表。DeepSeek Adapter 用一次官方 JSON Chat Completion 发送连续帧、模特图、商品图和只读 Skill；`director` 随后执行原 Schema、结构差异、时间线连续性和 prompt 完整性校验。`video-provider` 提供 `createTask/getTaskStatus/getResult`。
